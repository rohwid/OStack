...
keystone_pkg() {
  ...
  keystone-manage bootstrap --bootstrap-password ${KEYSTONE_ADMINPASS} \
...
openrc() {
  read -n1 -r -p "Create openrc varaible on '$(hostname)'. press ENTER to continue!" ENTER

  echo "[OSTACK] Configuring init-openrc.."
  sudo cp ../config/init-openrc ~

  echo "[OSTACK] Configuring admin-openrc.."
  sudo cp ../config/admin-openrc ~
