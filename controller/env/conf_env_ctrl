...
memcached() {
  ...
  if [[ -f /etc/memcached.conf ]]; then
    ...
    if [[ -f /etc/memcached.conf.ori ]]; then
      echo "[OSTACK] Restoring memcached original backup.."
      cp /etc/memcached.conf.ori /etc/memcached.conf
      echo "[OSTACK] Creating memcached temporary backup.."
      sed -i.bak -e "35d" /etc/memcached.conf
      echo "[OSTACK] Configuring memcached.."
      sed -i "35i -l ${IP_ADDR0_ETH0}" /etc/memcached.conf
    else
      echo "[OSTACK] Creating memcached original backup.."
